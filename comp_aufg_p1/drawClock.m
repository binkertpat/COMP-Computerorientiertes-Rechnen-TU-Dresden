function drawClock(h,m,s)
    ...
    theta = (90 - (360/12 * (h + (m / 60)  + (s / 3600)))) * pi/180;
    xCoordHourPointer =(r - 0.5) * cos(theta); 
    xCoordsHourPointer = [0 xCoordHourPointer];
    yCoordHourPointer =(r - 0.5) * sin(theta); 
    yCoordsHourPointer = [0 yCoordHourPointer];
    plot(xCoordsHourPointer,yCoordsHourPointer,'linewidth',3,'color',"r");
    
    %drawing minute pointer
    theta = (90 - (360/60 * (m + (s / 60)))) * pi/180;
    xCoordMinutePointer =(r - 3) * cos(theta); 
    xCoordsMinutePointer = [0 xCoordMinutePointer];
    yCoordMinutePointer =(r - 3) * sin(theta); 
    yCoordsMinutePointer = [0 yCoordMinutePointer];
    plot(xCoordsMinutePointer,yCoordsMinutePointer,'linewidth',2,'color',"r");
    ...
    plot(xCoordsSecondPointer,yCoordsSecondPointer,'linewidth',1,'color',"r");
end
